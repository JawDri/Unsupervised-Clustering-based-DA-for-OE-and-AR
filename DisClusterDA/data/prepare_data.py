...
class PytorchDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return self.imgs[idx].view(1, 32), self.tgts[idx]##########
...
def generate_dataloader(args):
    ...
    Source_train = pd.read_csv("/content/drive/MyDrive/DisClusterDA/data/Source_train.csv")
    
    Source_train = PytorchDataSet(Source_train)
    
    

    Source_test = pd.read_csv("/content/drive/MyDrive/DisClusterDA/data/Source_test.csv")
    #print(Source_test.index.values)
    Source_test = PytorchDataSet(Source_test)

    Target_train = pd.read_csv("/content/drive/MyDrive/DisClusterDA/data/Target_train.csv")
    Target_train = PytorchDataSet(Target_train)


    Target_test = pd.read_csv("/content/drive/MyDrive/DisClusterDA/data/Target_test.csv")
