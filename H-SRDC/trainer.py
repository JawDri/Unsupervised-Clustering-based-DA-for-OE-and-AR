...
def train(train_loader_source, train_loader_source_batch, train_loader_target, train_loader_target_batch, model, att_module, bws, bwt, gamma, beta, criterion, criterion_mec, optimizer, optimizer_att, itern, epoch, new_epoch_flag, src_weight, args):
    ...
    if args.src_cls: 
        ...
        input_source_var = Variable(input_source).cpu()
        target_source = target_source.cpu()###.cuda(non_blocking=True)
        ...
        prec1 = accuracy(ca_s, target_source, args.num_classes)
    ...
    if args.learn_embed:
        if args.src_cls:
            mu_k = att_module(torch.cat((bwt(f_t).cpu(), bws(f_s).cpu()), dim=0) * gamma + beta) # cluster centroid learner forward
...
def TarClusterLoss(args, epoch, output, target, softmax=True): # compute clustering loss on target batch
    ...
    #print(prob_q)
    loss = - (prob_q.cpu() * (prob_p.cpu() + args.eps).log()).sum(1).mean()
...
def SrcClassifyLoss(args, output, target, index, src_weight, lam, softmax=True, soft_label=False): # compute classification loss on source batch
    ...
    if args.src_mix_weight:
        src_weight = lam * src_weight[index] + (1 - lam) * torch.ones(output.size(0)).cpu()###.cuda()
    else:
        src_weight = src_weight[index].cpu()
    
    loss = - (src_weight * (prob_q.cpu() * (prob_p.cpu() + args.eps).log()).sum(1)).mean()
...
def validate(val_loader, model, criterion, epoch, args):
    ...
    for i, (input, target, _) in enumerate(val_loader):
        ...
        target_var = Variable(target).cpu()

        # compute output
        with torch.no_grad():
            output = model(input_var)[-1].cpu()
            loss = criterion(output, target_var)

        # measure accuracy and record loss
        prec1 = accuracy(output, target, args.num_classes)
        prec1fscore = faccuracy(output, target, args.num_classes)
        #prec1fscore = f1_score(target.cpu(), output.cpu(), average='weighted')
        total_vector[0], correct_vector[0] = accuracy_for_each_class(output, target, total_vector[0], correct_vector[0])
        losses.update(loss.item(), input.size(0))
        top1['cls'].update(prec1, input.size(0))
        ...
        if i % args.print_freq == 0:
            print('Test on target test set - [{0}][{1}/{2}]\t'
                  ...
                  cls_loss=losses, cls=top1['cls'], topfscore = topfscore['clsfscore']))
...
def validate_compute_cen(val_loader_target, val_loader_source, model, criterion, epoch, args, compute_cen=True):
    ...
    source_features = torch.FloatTensor(len(val_loader_source.dataset.imgs), model.module.feature_dim).fill_(0)
    source_targets = torch.LongTensor(len(val_loader_source.dataset.imgs)).fill_(0)
    c_src = torch.FloatTensor(args.num_classes, model.module.feature_dim).fill_(0)
    count_s = torch.FloatTensor(args.num_classes, 1).fill_(0)
    ...
    for i, (input, target, index) in enumerate(val_loader_target):  # iterarion in the target dataset
        ...
        for j in range(input.size(0)):
            if (args.cluster_method == 'spherical_kmeans'):
                ...
                c_tar[idx_max_score[j]] += feature[j].cpu()
        ...
        if i % args.print_freq == 0:
            print('Test on target training set - [{0}][{1}/{2}]\t'
                  ...
                  'T@1 {cls.val:.3f} ({cls.avg:.3f})\t'
                  'topfscore {topfscore.val:.3f} ({topfscore.avg:.3f})'.format(
                   epoch, i, len(val_loader_target), batch_time=batch_time,
                   data_time=data_time, cls_loss=losses, cls=top1['cls'], topfscore = topfscore['clsfscore']))
...
def k_means(target_features, target_targets, train_loader_target, epoch, c, args, best_prec, change_target=True):
    ...
    for itr in range(args.cluster_iter):
        ...
        prec1 = accuracy(-1 * dist_xt_ct, target_targets, args.num_classes)
        if prec1 > best_prec:
            best_prec = prec1
...
def spherical_k_means(target_features, target_targets, train_loader_target, epoch, c, args, best_prec, change_target=True):
    ...
    for itr in range(args.cluster_iter):
        ...
        prec1 = accuracy(-1 * dist_xt_ct, target_targets, topk=(1,))
        if prec1 > best_prec:
            best_prec = prec1
        total_vector_dist = torch.FloatTensor(args.num_classes).fill_(0)
        correct_vector_dist = torch.FloatTensor(args.num_classes).fill_(0)
