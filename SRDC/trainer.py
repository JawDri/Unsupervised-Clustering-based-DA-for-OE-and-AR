...
def train(train_loader_source, train_loader_source_batch, train_loader_target, train_loader_target_batch, model, learn_cen, learn_cen_2, criterion_cons, optimizer, itern, epoch, new_epoch_flag, src_cs, args):
    ...
    # prepare target data
    #print(next(iter(train_loader_target_batch))[1])
    try:
    ...
    target_target = target_target########.cuda(non_blocking = True)#######cuda
    #print(target_target)
    ...
    loss += weight * TarDisClusterLoss(args, epoch, ca_t.cpu(), target_target.cpu(), em=(args.cluster_method == 'em'))
    
    if args.learn_embed:
        prob_pred = (1 + (f_t.unsqueeze(1).cpu() - learn_cen.unsqueeze(0)).pow(2).sum(2).cpu() / args.alpha).pow(- (args.alpha + 1) / 2)
        loss += weight * TarDisClusterLoss(args, epoch, prob_pred, target_target, softmax=args.embed_softmax)
        if not args.no_second_embed:
            prob_pred_2 = (1 + (f_t_2.unsqueeze(1).cpu() - learn_cen_2.unsqueeze(0)).pow(2).sum(2).cpu() / args.alpha).pow(- (args.alpha + 1) / 2)
            loss += weight * TarDisClusterLoss(args, epoch, prob_pred_2.cpu(), target_target.cpu(), softmax=args.embed_softmax)
    ...
    if args.src_cls:
        ...
        target_source = target_source########.cuda(non_blocking = True)######cuda
        input_source_var = Variable(input_source)########.cuda(non_blocking = True)
        target_source_var = Variable(target_source)###########.cuda(non_blocking = True)
        
        # model forward on source
        f_s, f_s_2, ca_s = model(input_source_var)
        prec1_s = accuracy(ca_s.data, target_source, args.num_classes)
        ...
        if args.learn_embed:
            prob_pred = (1 + (f_s.unsqueeze(1).cpu() - learn_cen.unsqueeze(0).cpu()).pow(2).sum(2) / args.alpha).pow(- (args.alpha + 1) / 2)
            loss += weight * SrcClassifyLoss(args, prob_pred, target_source, index, src_cs, lam, softmax=args.embed_softmax, fit=args.src_fit)
            if not args.no_second_embed:
                prob_pred_2 = (1 + (f_s_2.unsqueeze(1).cpu() - learn_cen_2.unsqueeze(0).cpu()).pow(2).sum(2) / args.alpha).pow(- (args.alpha + 1) / 2)
...
def TarDisClusterLoss(args, epoch, output, target, softmax=True, em=False):
    ...
    if em:
        ...
        prob_q1 = Variable(torch.FloatTensor(prob_p.size()).fill_(0))#######cuda
        #print(target)
        prob_q1.scatter_(1, target.unsqueeze(1), torch.ones(prob_p.size(0), 1)) # assigned pseudo labels#########cuda
        if (epoch == 0) or args.ao:
            prob_q = prob_q1
        else:
            prob_q2 = prob_p / prob_p.sum(0, keepdim=True).pow(0.5)
            prob_q2 /= prob_q2.sum(1, keepdim=True)
            prob_q = (1 - args.beta) * prob_q1.cpu() + args.beta * prob_q2.cpu()
    
    if softmax:
        loss = - (prob_q.cpu() * F.log_softmax(output.cpu(), dim=1)).sum(1).mean()
    else:
        loss = - (prob_q.cpu() * prob_p.log().cpu()).sum(1).mean()
...
def SrcClassifyLoss(args, output, target, index, src_cs, lam, softmax=True, fit=False):
    if softmax:
        ...
        prob_p = output / output.sum(1, keepdim=True)
    prob_q = Variable(torch.FloatTensor(prob_p.size()).fill_(0))#######cuda
    ...
    if softmax:
        loss = - (src_weights.cpu() * (prob_q.cpu() * F.log_softmax(output.cpu(), dim=1)).sum(1)).mean()
    else:
        loss = - (src_weights.cpu() * (prob_q.cpu() * prob_p.log().cpu()).sum(1)).mean()
...
def validate(val_loader, model, criterion, epoch, args):
    ...
    for i, (input, target, _) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            loss = criterion(output.cpu(), target_var.cpu())

        # compute and record loss and accuracy
        prec1 = accuracy(output.data.cpu(), target.cpu(), args.num_classes)
        prec5 = prec1
        prec1fscore = faccuracy(output, target, args.num_classes)
        total_vector, correct_vector = accuracy_for_each_class(output.data.cpu(), target.cpu(), total_vector.cpu(), correct_vector.cpu()) # compute class-wise accuracy
        ...
        if i % args.print_freq == 0:
            print('Test on T test set - [{0}][{1}/{2}]\t'
                  ...
                  'topfscore {topfscore.val:.3f} ({topfscore.avg:.3f})'.format(
                   epoch, i, len(val_loader), batch_time=batch_time, 
                   loss=losses, top1=top1, top5=top5, topfscore = topfscore['clsfscore']))
...
def validate_compute_cen(val_loader_target, val_loader_source, model, criterion, epoch, args, compute_cen=True):
    ...
    source_features = torch.FloatTensor(len(val_loader_source.dataset.imgs), 8192).fill_(0)#####cuda8192/2048
    source_features_2 = torch.FloatTensor(len(val_loader_source.dataset.imgs), args.num_neurons*4).fill_(0)
    source_targets = torch.LongTensor(len(val_loader_source.dataset.imgs)).fill_(0)
    c_src = torch.FloatTensor(args.num_classes, 8192).fill_(0)####8192/2048
    c_src_2 = torch.FloatTensor(args.num_classes, args.num_neurons*4).fill_(0)
    count_s = torch.FloatTensor(args.num_classes, 1).fill_(0)######cuda
    if compute_cen:
        for i, (input, target, index) in enumerate(val_loader_source): # the iterarion in the source dataset
            input_var = Variable(input)
            target = target##########.cuda(non_blocking = True)
            #print('HEYYYYYYYYYYYYYYYYYYYYYY',index)

            with torch.no_grad():
                feature, feature_2, output = model(input_var)
            source_features[index, :] = feature.data.clone().cpu()
            source_features_2[index, :] = feature_2.data.clone().cpu()
            source_targets[index] = target.clone().cpu()
            target_ = torch.FloatTensor(output.size()).fill_(0)
            ...
            if args.cluster_method == 'spherical_kmeans':
                ...
                c_src += (feature.unsqueeze(1).cpu() * target_.unsqueeze(2).cpu()).sum(0)
                c_src_2 += (feature_2.unsqueeze(1).cpu() * target_.unsqueeze(2).cpu()).sum(0)
                count_s += target_.sum(0).unsqueeze(1).cpu()
    
    target_features = torch.FloatTensor(len(val_loader_target.dataset.imgs), 8192).fill_(0)########2048/8192
    target_features_2 = torch.FloatTensor(len(val_loader_target.dataset.imgs), args.num_neurons*4).fill_(0)
    target_targets = torch.LongTensor(len(val_loader_target.dataset.imgs)).fill_(0)
    pseudo_labels = torch.FloatTensor(len(val_loader_target.dataset.imgs), args.num_classes).fill_(0)    
    c_tar = torch.FloatTensor(args.num_classes, 8192).fill_(0)#####8192/2048
    c_tar_2 = torch.FloatTensor(args.num_classes, args.num_neurons*4).fill_(0)
    count_t = torch.FloatTensor(args.num_classes, 1).fill_(0)
    ...
    for i, (input, target, index) in enumerate(val_loader_target): # the iterarion in the target dataset
        ...
        target_features[index, :] = feature.data.clone().cpu() # index:a tensor 
        target_features_2[index, :] = feature_2.data.clone().cpu()
        target_targets[index] = target.clone().cpu()
        pseudo_labels[index, :] = output.data.clone().cpu()
            
        if compute_cen: # compute target class centroids
            pred = output.data.max(1)[1]
            pred_ = torch.FloatTensor(output.size()).fill_(0)
            pred_.scatter_(1, pred.unsqueeze(1).cpu(), torch.ones(output.size(0), 1))
            if args.cluster_method == 'spherical_kmeans':
                ...
                c_tar += (feature.unsqueeze(1).cpu() * pred_.unsqueeze(2).cpu()).sum(0)
                c_tar_2 += (feature_2.unsqueeze(1).cpu() * pred_.unsqueeze(2).cpu()).sum(0)
                count_t += pred_.sum(0).unsqueeze(1).cpu()
        ...
        if i % args.print_freq == 0:
            print('Test on T training set - [{0}][{1}/{2}]\t'
                  ...
                  'L {tc_loss.val:.4f} ({tc_loss.avg:.4f})\t'
                  'topfscore {topfscore.val:.3f} ({topfscore.avg:.3f})'.format(
                   epoch, i, len(val_loader_target), batch_time=batch_time,
                   data_time=data_time, tc_top1=top1, tc_top5=top5, tc_loss=losses, topfscore = topfscore['clsfscore']))

    # compute global class centroids
    c_srctar = torch.FloatTensor(args.num_classes, 8192).fill_(0)######2048/8192
    c_srctar_2 = torch.FloatTensor(args.num_classes, args.num_neurons*4).fill_(0)#####cuda
    if (args.cluster_method == 'spherical_kmeans'):
...
def source_select(source_features, source_targets, target_features, pseudo_labels, train_loader_source, epoch, cen, args):
    ...
    if args.src_hard_select:
        num_select_src_each_class = torch.LongTensor(args.num_classes).fill_(0)######cuda
        ...
        src_cs = torch.FloatTensor(len(train_loader_source.dataset.tgts)).fill_(1)
...
def kernel_k_means(target_features, target_targets, pseudo_labels, train_loader_target, epoch, model, args, best_prec, change_target=True):
    ...
    count = torch.FloatTensor(args.num_classes, 1).fill_(0)####cuda
...
def k_means(target_features, target_targets, train_loader_target, epoch, model, c, args, best_prec, change_target=True):
    ...
    for itr in range(args.cluster_iter):
        ...
        prec1 = accuracy(-1 * dist_xt_ct.data, target_targets, args.num_classes).item()
        is_best = prec1 > best_prec
        if is_best:
            best_prec = prec1
...
def spherical_k_means(target_features, target_targets, train_loader_target, epoch, model, c, args, best_prec, change_target=True):
    ...
    for itr in range(args.cluster_iter):
        ...
        if args.src.find('visda') != -1:
            ...
            log.write("\n                          Avg_dist. over all classes: %3f" % acc_for_each_class_dist.mean())
        log.close()
        c_tar_temp = torch.FloatTensor(args.num_classes, c_tar.size(1)).fill_(0)######cuda
        for k in range(args.num_classes):
